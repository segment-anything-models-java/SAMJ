...
public class EfficientTamJ extends AbstractSamJ {
	...
	private static final String IMPORTS = ""
			...
			+ "predictor.memory_temporal_stride_for_eval=2" + System.lineSeparator() // TODO what do we do here
			+ "predictor.max_obj_ptrs_in_encoder=8" + System.lineSeparator() // TODO what do we do here
			...
			+ "" + System.lineSeparator()
			+ "def _slice_backbone_out(backbone_out, idx):" + System.lineSeparator()
			+ "    return {" + System.lineSeparator()
			+ "        'backbone_fpn': [feat[idx:idx + 1].contiguous() for feat in backbone_out['backbone_fpn']]," + System.lineSeparator()
			+ "        'vision_pos_enc': [pos[idx:idx + 1].contiguous() for pos in backbone_out['vision_pos_enc']]," + System.lineSeparator()
			+ "    }" + System.lineSeparator()
			+ "" + System.lineSeparator()
			+ "def precompute_backbone_features(inference_state, frame_indices=None, batch_size=None):" + System.lineSeparator()
			+ "    frame_cache = inference_state.setdefault('cached_backbone', OrderedDict())" + System.lineSeparator()
			+ "    num_frames = inference_state['num_frames']" + System.lineSeparator()
			+ "    if frame_indices is None:" + System.lineSeparator()
			+ "        frame_indices = list(range(num_frames))" + System.lineSeparator()
			+ "    else:" + System.lineSeparator()
			+ "        frame_indices = [int(idx) for idx in frame_indices]" + System.lineSeparator()
			+ "    missing = [idx for idx in frame_indices if idx not in frame_cache]" + System.lineSeparator()
			+ "    if len(missing) == 0:" + System.lineSeparator()
			+ "        return" + System.lineSeparator()
			+ "    if batch_size is None:" + System.lineSeparator()
			+ "        batch_size = 1 if predictor.device.type == 'cpu' else 2" + System.lineSeparator()
			+ "    batch_size = max(1, int(batch_size))" + System.lineSeparator()
			+ "    for start in range(0, len(missing), batch_size):" + System.lineSeparator()
			+ "        batch_indices = missing[start:start + batch_size]" + System.lineSeparator()
			+ "        batch_images = inference_state['images'][batch_indices].to(inference_state['device']).float()" + System.lineSeparator()
			+ "        batch_backbone_out = predictor.forward_image(batch_images)" + System.lineSeparator()
			+ "        for local_idx, frame_idx in enumerate(batch_indices):" + System.lineSeparator()
			+ "            frame_cache[frame_idx] = _slice_backbone_out(batch_backbone_out, local_idx)" + System.lineSeparator()
			+ "" + System.lineSeparator()
			+ "def _cached_get_image_feature(self, inference_state, frame_idx, batch_size):" + System.lineSeparator()
			+ "    frame_cache = inference_state.setdefault('cached_backbone', OrderedDict())" + System.lineSeparator()
			+ "    backbone_out = frame_cache.get(frame_idx)" + System.lineSeparator()
			+ "    if backbone_out is None:" + System.lineSeparator()
			+ "        precompute_backbone_features(inference_state, [frame_idx], batch_size=1)" + System.lineSeparator()
			+ "        backbone_out = frame_cache[frame_idx]" + System.lineSeparator()
			+ "    image = inference_state['images'][frame_idx].to(inference_state['device']).float().unsqueeze(0)" + System.lineSeparator()
			+ "    expanded_image = image.expand(batch_size, -1, -1, -1)" + System.lineSeparator()
			+ "    expanded_backbone_out = {" + System.lineSeparator()
			+ "        'backbone_fpn': [feat.expand(batch_size, -1, -1, -1) for feat in backbone_out['backbone_fpn']]," + System.lineSeparator()
			+ "        'vision_pos_enc': [pos.expand(batch_size, -1, -1, -1) for pos in backbone_out['vision_pos_enc']]," + System.lineSeparator()
			+ "    }" + System.lineSeparator()
			+ "    features = self._prepare_backbone_features(expanded_backbone_out)" + System.lineSeparator()
			+ "    return (expanded_image,) + features" + System.lineSeparator()
			+ "" + System.lineSeparator()
			+ "predictor._get_image_feature = types.MethodType(_cached_get_image_feature, predictor)" + System.lineSeparator()
			+ "" + System.lineSeparator()
			...
			+ "    inference_state[\"cached_backbone\"] = OrderedDict()" + System.lineSeparator()
			...
			+ ""  + System.lineSeparator()
			+ ""
			+ "task.export(precompute_backbone_features=precompute_backbone_features)" + System.lineSeparator()
	...
	protected void processBoxWithSAMAndPropagate() {
		...
		String code = "" + System.lineSeparator()
				...
				+ "initial_mask_area = int(mask[0].sum())" + System.lineSeparator()
				+ "min_mask_area_for_presence = max(8, initial_mask_area // 200)" + System.lineSeparator()
				+ "max_consecutive_missing_frames = 2" + System.lineSeparator()
				+ "" + System.lineSeparator()
				...
				+ "    max_frame_num_to_track = min(" + DEFAULT_MAX_PROPAGATION_RADIUS + ", end_frame_idx - seed_frame_idx)" + System.lineSeparator()
				+ "    missing_streak = 0" + System.lineSeparator()
				...
				+ "        mask = (mask[0] > 0.0).cpu().numpy()" + System.lineSeparator()
				+ (this.isIJROIManager ? "        mask[0, 1:, 1:] += mask[0, :-1, :-1]" : "") + System.lineSeparator()
				+ "        mask_area = int(mask[0].sum())" + System.lineSeparator()
				+ "        if mask_area < min_mask_area_for_presence:" + System.lineSeparator()
				+ "            missing_streak += 1" + System.lineSeparator()
				+ "            if missing_streak >= max_consecutive_missing_frames:" + System.lineSeparator()
				...
				+ "    max_frame_num_to_track = min(" + DEFAULT_MAX_PROPAGATION_RADIUS + ", seed_frame_idx)" + System.lineSeparator()
				+ "    missing_streak = 0" + System.lineSeparator()
				+ "    for frame_idx, obj_ids, mask in predictor.propagate_in_video(" + System.lineSeparator()
				+ "        state," + System.lineSeparator()
				+ "        start_frame_idx=seed_frame_idx," + System.lineSeparator()
